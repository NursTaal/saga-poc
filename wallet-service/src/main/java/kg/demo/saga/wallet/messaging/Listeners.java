...
public class Listeners {
  private final WalletService service; public Listeners(WalletService s){this.service=s;}

  // Orchestration: команда на дебет
  @RabbitListener(queues = "wallet.debit.q")
  public void onDebit(DebitCommand cmd){ service.handleDebit(cmd.sagaId(), cmd.userId(), cmd.txnId(), cmd.amount(), cmd.failDebit()); }

  // Choreography: реагируем на созданную транзакцию
  @RabbitListener(queues = "wallet.on.txCreated.q")
  public void onTxCreated(TransactionCreated evt){ service.handleDebit(evt.sagaId(), evt.userId(), evt.txnId(), evt.amount(), evt.failDebit()); }
}
